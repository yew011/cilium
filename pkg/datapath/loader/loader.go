...
func patchHostNetdevHeaderFile(stateDir, ifName string, mac mac.MAC, ifIndex uint32,
	secctxFromIpcache uint32, nodePortIPv4, nodePortIPv6 net.IP,
	nodePort bool) (netdevHeaderFile string, err error) {
...
func patchHostNetdevDatapath(ep datapath.Endpoint, objPath, dstPath, ifName string,
	...
	netdevHeaderFile, err := patchHostNetdevHeaderFile(ep.StateDir(), ifName,
		mac, ifIndex, opts["SECCTX_FROM_IPCACHE"], nodePortIPv4, nodePortIPv6, nodePort)
	if err != nil {
		return err
	}
